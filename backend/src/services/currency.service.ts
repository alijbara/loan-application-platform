...
export class CurrencyService implements ICurrencyService {
  ...
  constructor(apiService: IApiService, cacheService: ICacheService) {
    ...
    if (!url) throw new Error('No Currency API key was provided in environment variables');
    this._url = url;
    // set ttl
  ...
  private async fetchExchangeRate(fromCurrency: Currency, toCurrency: Currency): Promise<number> {
    const url = `${this._url}/latest?apikey=${this._apiKey}&base_currency=${fromCurrency}&currencies=${toCurrency}`;
